		...
		var _addCloseButton = function ($el) {
			...
			$('<span class="lobibox-close">&times;</span>')
				.click(function (ev) {
					ev.preventDefault();
					ev.stopPropagation();
					if(me.$options.onCloseButtonClick != null)
						me.$options.onCloseButtonClick();
					me.remove();
					
				}).appendTo($el);
		}; 
		var _addCustomButton = function ($el) {
			if (!me.$options.hasExtraButton) {
				return;
			}
			$('<span class="lobibox-customebutton">'+me.$options.extraButtonIcon+'</span>')
				.click(function (ev) {
					ev.preventDefault();
					ev.stopPropagation();
					if(me.$options.onExtraButtonClick != null)
						me.$options.onExtraButtonClick();
					if (me.$options.extraButtonClickClose)
						me.remove();
				}).appendTo($el);
		};
...
//------------------------------------------------------------------------------
	...
	Lobibox.notify.DEFAULTS = {
		...
		hasExtraButton: false,			 // Make notifications closable
		extraButtonClickClose: true,			 // Make notifications closable
		onCloseButtonClick: null,			 // Make notifications closable
		onExtraButtonClick: null,			 // Make notifications closable
		extraButtonIcon: '<i class="glyphicon glyphicon-pencil"></i>',			 // Make notifications closable
